#-------------------------------------------------------------
##		Working with distributions, Bayes, SQL
#-------------------------------------------------------------
...
library(sqldf)
acs <- read.csv('E:\\Coursera\\SampleData.csv')
head(acs)
...
subset(farms, Manag %in% c("BF", "HF"))
sqldf("
SELECT *
FROM farms
WHERE Manag IN ('BF', 'HF')
")

subset(warpbreaks, breaks >= 20 & breaks <= 30)
sqldf("
SELECT *
FROM warpbreaks
